...
## Pages whose content still lives in future milestones resolve to a themed
## placeholder page with the same title, so navigation is testable end-to-end.
...
const NARROW_BREAKPOINT := 960.0

## Keyboard / mouse / gamepad labels for the footer prompt bar, indexed as
## accept, back, details, tab.
const PROMPT_LABELS: Dictionary = {
	&"keyboard": ["ENTER  ACCEPT", "ESC  BACK", "TAB  DETAIL", "SHIFT+TAB  PREV"],
	&"mouse": ["CLICK  ACCEPT", "ESC  BACK", "HOVER  DETAIL", "NAV RAIL"],
...
func _make_placeholder_page(page_id: StringName) -> Control:
	...
	title.tooltip_text = "%s — coming in a later milestone" % PAGE_DISPLAY_NAMES[page_id]
	box.add_child(title)
	var body := Label.new()
	body.text = "No section content yet. Use ESC or the navigation rail to return to the Command Deck."
...
func _cycle_page(direction: int) -> void:
	...
	if index < 0:
		index = 0
	var next_index := (index + direction + PAGE_NAV_ORDER.size()) % PAGE_NAV_ORDER.size()
	show_page(PAGE_NAV_ORDER[next_index])
...
func _load_reduced_flashing_setting() -> bool:
	...
	if save_manager != null and save_manager.has_method("get_setting"):
		return bool(save_manager.call("get_setting", "reduced_flashing", false))
	return false
