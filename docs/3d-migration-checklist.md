...
- [x] Port Player Craft life damage and source-specific invulnerability for native Enemy Craft contact and Enemy Projectiles.
- [x] Port projectile-dependent boost deflection and chain behavior; keep it separate from ordinary damage routing.
...
- **Manual gate at approval:** `res://scenes/basic_enemy_3d_review.tscn` was reviewed for Gen I silhouette/color, cardinal facing, movement, hitbox and socket alignment, firing hits/misses, contacts, cleanup, pause, and restart. Keys **1–4** spawned from top/right/bottom/left, **V** toggled automatic entries, **H** toggled hitboxes, and **G** toggled sockets; flight/fire/boost controls were unchanged. At that gate, destroyed, contact, and escape counters each reflected their own outcome once while score and lives deliberately did not change.

### Phase 3 Generation I Basic Enemy approval record

- The user's 2026-08-28 continuation approves moving past the Generation I Basic Enemy gate. The next bounded slice covers native Player Craft life damage and invulnerability for Basic Enemy contact and pooled Enemy Projectile hits. Boost deflection/chains, shields, hostile ordnance, rewards/orbs, later enemy generations, VFX, menu asset loading, production cutover, and legacy cleanup remain deferred.

### Phase 3 Player damage and invulnerability validation record

- `PlayerDamageTuning` is now the spatially independent source for the existing reference timings: three seconds after Enemy Craft contact, two seconds after Enemy Projectile or future Hostile Ordnance damage, 1.5 seconds for the still-reference-only shield response, and a 0.12-second blink half-interval. The 2D Player reads the same constants without changing its damage behavior. Native Player damage consumes lives only through the existing synchronous `SignalBus.player_hit`/`GameManager` authority; no second life store or 3D-only score/reward state was introduced.
- `Player3D.receive_damage()` rejects hits while the run is inactive, lives are exhausted, or invulnerability is active. Accepted hits flatten their event position to the Combat Plane at `Y=0`, play the existing Player hit audio, consume exactly one life, and report the source and post-hit life count. Surviving hits start source-specific immunity; the fatal hit emits the existing game-over signal exactly once, leaves no immunity running, keeps the craft visible, and rejects further damage. The temporary native feedback blinks the wrapper's `Visuals` node; polished damage, shield, death, and impact effects remain in the later VFX slices.
- Native Enemy Projectile damage has one owner: `ProjectileManager3D` emits its existing hit event and `Native3DGameplay` routes Player targets into `Player3D`. The Projectile review's second listener is telemetry only. Live normal and deliberate 6× collision-stress volleys each produced one contact and one life loss when vulnerable; another volley during immunity produced a contact with no life loss. A later vulnerable volley consumed the next life. Pool growth remained zero and all reported hit positions stayed on `Y=0`.
- Native Generation I contact uses the existing Player/Enemy `Area3D` overlap. A vulnerable contact consumed one life and started a three-second window. An immediate second Enemy contact was still removed and counted but consumed no life; a third contact after expiry consumed the next life. Score remained zero, no rewards or orbs appeared, and the idle projectile pools did not grow. There is still no `Area2D`, ship `SubViewport` proxy, or co-loaded 2D gameplay actor in either review.
- The invulnerability timer, blink cadence, Player movement, active projectiles/enemies, review timers, and input all froze under the existing pause flow; resume completed the same immunity window without changing `Engine.time_scale`. The review-only **R Restore Lives** action despawns active incoming projectiles where applicable, resets run state through `GameManager.start_game(false)`, clears wrapper-local immunity, and restores three visible lives after a fatal hit. Production game-over/try-again presentation remains a cutover dependency rather than a review-scene addition.
- Godot 4.6.3 imported and ran both composed reviews on Metal Forward+. Automatic incoming volleys and enemy entries now default off so the reviewer is never killed unattended. Clean layouts at 1280×720 expose lives, accepted-damage count, and vulnerability state. Temporary probe mistakes were discarded; clean scene launches report no gameplay-code errors, while the generated MCP bridge retains its own shadowing/enum warnings.
- The existing ten-scene regression suite ran once with a verified, distinct `user://` profile. Seven scenes reported PASS. `enemy_evolution_shader_smoke` reproduced its known legacy `BaseEnemy` import failures without a PASS sentinel; `player_upgrade_3d_smoke` and `visual_upgrade_smoke` reproduced their known shared-shader identity failures. `balance_ttk_smoke` still reached PASS while printing its established legacy import diagnostics. No changed Player/native-damage script appeared in the failures, and no dedicated migration test suite or permanent diagnostic fixture was added. Logs and the isolated profile are retained at `/private/tmp/farinuff-player-damage-regression.Us4m60`; the live save and backup both retained SHA-256 `6c70248833464a39995ee83a6754d4836ab3cc33daebe9b21577284ecc992f5f`.
- The independent Spec review reported zero findings. The Standards review's initial lifecycle concern was resolved against this later, bounded checklist: the review scenes validate fatal run state and one-shot game-over signaling, while production presentation remains an explicit cutover dependency. Review follow-up removed unused native Hostile Ordnance source handling and preserved the typed `DamageSource` contract in telemetry callbacks. The remaining suggestion to extract two small review-only telemetry/reset blocks was retained as a documented judgment call; a new migration-only component would add more coupling than these temporary manual harnesses justify. No hard standards violations remain.
- **Manual gate:** run both `res://scenes/projectile_3d_review.tscn` and `res://scenes/basic_enemy_3d_review.tscn` directly. In the Projectile review, **1** fires a normal volley, **2** fires the 6× collision-stress volley, **V** toggles automatic volleys, **H** toggles range guides, and **R** restores the review run. In the Basic Enemy review, **1–4** spawn cardinal entries, **V** toggles automatic entries, **H** toggles hitboxes, **G** toggles sockets, and **R** restores the review run. Confirm vulnerable hits consume one life, repeat hits inside the displayed 2-second/3-second immunity do not, fatal damage stops the run without jitter or global slow motion, pause freezes the immunity window, and pool growth/score/rewards remain unchanged. Explicit user acceptance is required before boost deflection/chains or another migration slice.

### Phase 3 Player damage and invulnerability approval record

- The user's 2026-08-28 continuation approves moving past the Player damage and invulnerability gate. This next bounded slice covers pooled Enemy Projectile boost deflection, reflected-projectile Enemy Craft hits, reflection-adjusted cooldowns, and the three-reflection chain window. Shields, Hostile Ordnance, rewards/orbs, later enemy generations, VFX polish, menu asset loading, production cutover, and legacy cleanup remain deferred.

### Phase 3 boost deflection and chain validation record

- Native and reference Player Craft now read the same reflection cooldown values from `PlayerFlightTuning`: 0.85 seconds with no reflection, 0.35 seconds after one, then 0.08-second reductions down to a 0.10-second floor. Three reflections grant a zero-cooldown follow-up boost and a 0.18-second post-boost deflection window. A follow-up boost resets the reflection count and window. Successful reflections inside the post-boost window remain defensive and do not count toward another chain.
- `Player3D` owns the boost/chain state and synchronously requests a bounded Enemy Projectile scan before checking follow-up boost input, preserving the reference update order. `ProjectileManager3D` owns the 74-baseline-pixel range query and registers only successful reflections. A collision-time fallback preserves the reference defense when a Projectile crosses the whole range between scans or starts overlapped; the wrapper discards its stale inbound step when that synchronous contact is redirected. This remains separate from `Player3D.receive_damage()` and does not alter invulnerability or life authority.
- A reflected Enemy Projectile keeps its existing pooled wrapper but becomes Player-aligned: its direction is radial from the Player Craft with the shared 0.32 velocity bias, and its speed becomes the greater of 1.35× incoming speed or 560 baseline pixels/second. Its layer, mask, overlap target, and sweep target switch to Enemy Craft. Reflected hits reuse the existing Player Projectile damage route and play the existing hit-marker audio, so `Native3DGameplay` remains the single native Enemy damage authority.
- The Enemy Projectile wrapper authors the cyan/green reflected palette as scene-local data. Reflection applies those base, energy, accent, and glow colors through instance parameters on the same shared mesh, material, and shader. The material is warmed under the transition cover, and reflection performs no material duplication or shader compilation. Pool return restores the orange visual, incoming faction contract, collision state, motion, lifetime, and every material instance parameter before the wrapper becomes available again.
- `res://scenes/projectile_3d_review.tscn` now includes a hidden, inert, scene-authored Generation I target for transition warm-up. **3 CHAIN + TARGET** places the real target behind three tightly grouped incoming Projectiles; boosting through the burst exercises reflection counting, return-hit routing, and an actual target kill. The HUD separately reports ordinary hits, accepted Player damage, deflections, reflected hits, target kills, lives, vulnerability, and chain readiness.
- Live Godot 4.6.3/Metal Forward+ checks deflected all three review Projectiles, preserved three Player lives, registered one reflected hit and one Generation I kill with hit-marker audio, granted the zero-cooldown chain, and produced zero post-warm-up pool growth. A normal non-boosted hit still changed lives from three to two through the existing damage path. Reflections succeeded at 73 pixels and not at 80 pixels; the 6× review Projectile still reflected before contact. With the ordinary scan disconnected, a 12,000-pixels/second Projectile crossed from 120 pixels to the 22-pixel contact boundary and reflected outward without damage, while a spawn directly overlapping the Player reflected at the 560-pixels/second minimum. An off-axis crossing matched the actual impact-point radial direction exactly rather than the prior-frame origin. Exact end states were 0/1/2/3-reflection cooldowns of 0.85/0.35/0.27/0.00 seconds, with only the three-reflection case opening the 0.18-second window.
- Pause held projectile position/lifetime and the chain window fixed while `Engine.time_scale` remained 1. A saturated 64-Projectile reflection scan deflected all warmed nodes in approximately 1.101 ms on the local debug run, rejected the 65th activation, and produced zero pool growth. Returning all 64 wrappers left every Enemy Projectile idle with the orange visual restored and collision disabled. These bounded diagnostics are not a complete combat-performance benchmark.
- The existing ten-scene regression suite ran once against a verified isolated `user://` profile. Seven scenes reported PASS. `enemy_evolution_shader_smoke` retained its known legacy `BaseEnemy` import failures without a PASS sentinel; `player_upgrade_3d_smoke` and `visual_upgrade_smoke` retained their known shared-shader identity failures. `balance_ttk_smoke` still reached PASS while printing its established legacy import diagnostics. After review changed the shared toon-shader interface, the affected boss-render scene passed again and the two known shared-shader failures reproduced unchanged in targeted reruns. No changed native deflection script appeared in those failures, and no dedicated migration test suite or permanent diagnostic fixture was added. Logs and the isolated profile are retained at `/private/tmp/farinuff-boost-deflection-regression.mVBTUm`; the live save and backup retained their pre-run SHA-256 hashes (`49fc46d3860fdeb1d48baf09a22dca36a275e8bd03613d9e31d7d958f56a0c2f` and `8118f0cb0d1c56fbac381255fa866a5b951c289ab175b1167a9520eab6ccdff4`).
