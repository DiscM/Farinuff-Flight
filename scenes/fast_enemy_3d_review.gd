extends Node
class_name FastEnemy3DReview
## Manual Generation I Fast Enemy review around the actual native controller.
...
const FastEnemy := preload("res://entities/enemies/fast_enemy_3d.gd")
const ENEMY_SCENE := preload("res://entities/enemies/fast_enemy_3d.tscn")
...
@onready var _first_enemy: FastEnemy = $Gameplay/World3D/Actors3D/FirstEnemy3D
...
func spawn_from_edge(edge: int) -> void:
	...
	_spawn_count += 1
	enemy.name = "FastEnemy3D_%d" % _spawn_count
...
func _update_status() -> void:
	...
	status.text = "ACT %d/%d • KILL %d • CONTACT %d • ESC %d • DAMAGE %d • LIVES %d/%d • WEAVE 80 PX @ 3 HZ • %s" % [
