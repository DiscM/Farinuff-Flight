...
## except for TRACK targeting, which refreshes its aim at a capped rate until
## release. Targeting and style are chosen by the combat FSM at commitment.
const Definition := preload("res://systems/boss_attack_definition.gd")

enum Targeting { LEAD, SNAP, TRACK, BRACKET }
